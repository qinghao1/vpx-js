...
for (const [k,v] of Object.entries(t.flashers)) {
  const d=v.data;
  if (d.isDMD) {
    console.log('Flasher',k,'isDMD',d.isDMD,'center',JSON.stringify(d.center),'height',d.height,'pts',d.dragPoints?.map(p=>`(${p.vertex.x},${p.vertex.y})`).join(' '));
    const xs=d.dragPoints.map(p=>p.vertex.x), ys=d.dragPoints.map(p=>p.vertex.y);
