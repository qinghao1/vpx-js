...
function exists(p) { try { return fs.existsSync(p) && fs.statSync(p).size > 1024*1024 } catch { return false } }
...
const group = await table.generateTableNode(api, {
	exportPlayfield: true, exportPrimitives: true, exportFlippers: true, exportBumpers: true,
	exportRamps: true, exportSurfaces: true, exportRubbers: true, exportLightBulbs: true,
	exportHitTargets: true, exportGates: true, exportKickers: true, exportTriggers: true,
	exportSpinners: true, exportPlungers: true, preloadTextures: false,
}) as THREE.Group
group.updateMatrixWorld(true)
const logs=[]
postProcessScene(group, { harnessLog: (m)=>logs.push(m), viewerMode: 'viewer' })
console.log(logs.join(' | '))
function isEff(o, root) { if (!o.visible) return false; for (let p=o.parent; p&&p!==root; p=p.parent) if (!p.visible) return false; return true }
let underwallEff=0
group.traverse(o=>{ if (o.isMesh && (o.name||'').toLowerCase().includes('playfield_underwall') && isEff(o, group)) underwallEff++ })
console.log(`underwall effective visible: ${underwallEff} => ${underwallEff===0 ? 'PASS' : 'FAIL (vertical artifact)'}`)
let pendingBMVisible=0
group.traverse(o=>{ if (!o.isMesh || !isEff(o,group)) return; const n=(o.name||'').toLowerCase(); if (!n.includes('bm_') && !n.includes('playfield')) return; const m=Array.isArray(o.material)?o.material[0]:o.material; if (!m) return; const pending=m.userData.pendingMap||m.userData.pendingmap; if (pending) pendingBMVisible++ })
console.log(`pending BM visible (should be 0 after postProcess for BM): ${pendingBMVisible} => ${pendingBMVisible===0 ? 'PASS' : 'FAIL'}`)
// check su/sling surface not incorrectly forced
let surfaceVisible=0
group.traverse(o=>{ if (o.isMesh && o.name.includes('Playfield_underwall') && o.visible) surfaceVisible++ })
