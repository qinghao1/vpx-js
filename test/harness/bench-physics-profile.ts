...
const vpx = getArg('vpx', null) ?? (fs.existsSync('walking_dead.vpx') ? 'walking_dead.vpx' : 'test/fixtures/table-empty.vpx')
...
function stats(arr: number[]) {
	...
	return { avg: +avg.toFixed(3), p50: +s[(s.length * 0.5) | 0].toFixed(3), p95: +s[(s.length * 0.95) | 0].toFixed(3), p99: +s[(s.length * 0.99) | 0].toFixed(3), min: +Math.min(...s).toFixed(3), max: +Math.max(...s).toFixed(3) }
}
...
console.log(`[profile] Table.load ${(performance.now() - t0).toFixed(0)}ms  Game=${(table as any).tableScript?.match(/cGameName\s*=\s*["']([^"']+)["']/i)?.[1] ?? 'n/a'}`)
let wasmReady = false
try { await getWasmKernels(); wasmReady = isWasmReady(); } catch {}
...
console.log(`[profile] hitObjects=${phys.hitObjects?.length ?? '?'}  hitObjectsDynamic=${phys.hitObjectsDynamic?.length ?? '?'}`)

const w = (table as any).data?.width ?? 1000, h = (table as any).data?.height ?? 2000
for (let i = 0; i < ballsArg; i++) {
	player.createBall({ getBallCreationPosition: () => new Vertex3D(w * 0.5 + (Math.random() - 0.5) * 200, h * 0.75 + (Math.random() - 0.5) * 200, 30), getBallCreationVelocity: () => new Vertex3D((Math.random() - 0.5) * 600, -200 - Math.random() * 300, 20), onBallCreated: () => {} }, 25, 1)
}
console.log(`[profile] balls=${player.balls.length}`)

let hitCalls = 0, wasmCalls = 0, scalarCalls = 0, totalOrderLen = 0
let maxOrderLen = 0
...
hitCalls = 0; wasmCalls = 0; scalarCalls = 0; totalOrderLen = 0; maxOrderLen = 0; orderHist.clear()

const origSim = phys.physicsSimulateCycle.bind(phys)
let innerHitMs = 0
phys.physicsSimulateCycle = function(dTime: number) {
...
phys.updateVelocities = function() {
	const a = performance.now()
	const r = origVel()
...
if (origFire) {
	(phys as any).fireTimers = function(mode: any) {
		const a = performance.now()
...
for (let i = 0; i < ticks; i++) {
	...
	if (i % 200 === 0 && i > 0) console.log(`  ${i}/${ticks}  avg ${(physTimes.slice(-200).reduce((a,b)=>a+b,0)/200).toFixed(3)}ms`)
}
...
console.log(`  inner physicsSimulateCycle total ${innerHitMs.toFixed(1)}ms avg ${(innerHitMs/ticks).toFixed(3)}ms/tick`)
console.log(`  hitTestBall calls=${hitCalls} wasmPath~${wasmCalls} scalarPath~${scalarCalls} avgOrderLen=${hitAvg.toFixed(1)} max=${maxOrderLen} hist=${JSON.stringify(Object.fromEntries(orderHist))}`)
console.log(`  WASM ready=${wasmReady}  threshold=64  balls=${player.balls.length}  hitObjects=${phys.hitObjects.length}`)
...
console.log(`  micro hitCalls=${hitCalls} wasm~${wasmCalls} scalar~${scalarCalls} avgOrder=${hitCalls? (totalOrderLen/hitCalls).toFixed(1):0}`)

console.log(`\n[varying balls]`)
for (const n of [1,5,10,20]) {
	const pl = new Player(table).init() as any
	for (let i = 0; i < n; i++) pl.createBall({ getBallCreationPosition: () => new Vertex3D(w*0.5 + Math.random()*10, h*0.5 + Math.random()*10, 30), getBallCreationVelocity: () => new Vertex3D((Math.random()-0.5)*200, (Math.random()-0.5)*200, 0), onBallCreated: ()=>{} }, 25, 1)
	try { await getWasmKernels() } catch {}
	let cur = 16
	pl.updatePhysics(cur)
	for (let i=0;i<50;i++){cur+=16; pl.updatePhysics(cur)}
	const arr: number[]=[]
	cur += 16
	for(let i=0;i<200;i++){cur+=16; const a=performance.now(); pl.updatePhysics(cur); arr.push(performance.now()-a)}
	console.log(`  balls=${n}  avg=${(arr.reduce((a,b)=>a+b,0)/arr.length).toFixed(3)}ms  p95=${stats(arr)?.p95}ms`)
}

if (hasFlag('json')) {
	const out = { vpx, balls: ballsArg, ticks, wasmReady, physStats: stats(physTimes), moverStats: stats(moverTimes), hitCalls, wasmCalls, scalarCalls, avgOrderLen: hitAvg, maxOrderLen, hist: Object.fromEntries(orderHist) }
