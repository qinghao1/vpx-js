...
export async function loadPuppeteer() {
  for (const s of ['puppeteer-core', 'puppeteer']) {
    try { const m = await import(s); return m.default ?? m } catch {}
  }
...
export async function ensureVite(url, { cwd = import.meta.dirname, label = 'vite' } = {}) {
  ...
  for (let attempt = 0; attempt < 3; attempt++) {
    for (const u of candidates) {
      try { if ((await fetch(u, { method: 'HEAD' })).ok) return null } catch {}
      try { if ((await fetch(u, { method: 'GET' })).ok) return null } catch {}
    }
    if (attempt < 2) { console.log(`[${label}] vite not ready, retry ${attempt+1}/3…`); await new Promise((r) => setTimeout(r, 800)) }
  }
  console.log(`[${label}] starting vite in ${cwd}…`)
  const proc = spawn('npx', ['vite', '--host', '--port', '3000', '--clearScreen', 'false'], { cwd, stdio: ['ignore','pipe','pipe'] })
  proc.stdout.on('data', (d) => process.stdout.write(`[${label}] ${d}`))
  proc.stderr.on('data', (d) => process.stderr.write(`[${label}] ${d}`))
  for (let i = 0; i < 30; i++) {
    await new Promise((r) => setTimeout(r, 1000))
    try { if ((await fetch('http://localhost:3000/', { method: 'HEAD' })).ok) { console.log(`[${label}] vite ready`); return proc } } catch {}
  }
