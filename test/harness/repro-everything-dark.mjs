...
console.log(
	`Avg point light intensity: ${avg.toFixed(2)} sample ${pointLights
		.slice(0, 3)
		.map(p => p.intensity)
		.join(', ')}`,
)
...
if (table.lights['106']) {
	for (const n of ['106', '107']) {
		...
		if (l)
			console.log(
				`${n} data state ${l.data.state} intensity ${l.data.intensity} LightState ${l.getState().intensity} isOn ${l.data.isOn()}`,
			)
	}
...
let pf = null
scene.traverse(o => {
	if (o.name === 'playfield_mesh') pf = o
})
if (pf) {
	const m = pf.children[0].material
	console.log(
		`playfield_mesh mat ${m.name} col ${m.color.getHexString()} em ${m.emissive.getHexString()} int ${m.emissiveIntensity} baked ${m.userData.__isBaked}`,
	)
