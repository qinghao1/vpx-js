...
function toWorldFromD3D(p){ return new Vertex3D(p.x, p.z, p.y) }
function toWorldFromRH(p){ return new Vertex3D(p.x, -p.z, p.y) }
...
async function harness(){
  ...
  try{
    ...
    const full = new Matrix3D().rotateXMatrix(cppAngle).multiply(new Matrix3D().rotateZMatrix(rot*Math.PI/180))
    const vert = full.clone().multiply(new Matrix3D().setScaling(len,len,len)).multiply(new Matrix3D().setTranslation(data.center.x,data.center.y,posZ))
  ...
  try{
    ...
    for(const side of ['pos','neg']){
      ...
      player2.createBall({
        getBallCreationPosition(){return new Vertex3D(sx,sy,30)},
        getBallCreationVelocity(){return new Vertex3D(vx,vy,0)},
        onBallCreated(){}
      })
      ...
      const vert0 = full0.clone().multiply(new Matrix3D().setScaling(len,len,len)).multiply(new Matrix3D().setTranslation(data.center.x,data.center.y,posZ))
      ...
      if(dot <= 0){
        console.log(`not ok 2 - spinner side ${side} deltaY ${deltaY.toFixed(2)} vy ${vy} dot ${dot.toFixed(2)} wrong`)
        ok=false
      } else {
        console.log(`# side ${side} angle ${angle.toFixed(3)} deltaY ${deltaY.toFixed(2)} dot ${dot.toFixed(2)} OK`)
      }
    }
    if(ok){
