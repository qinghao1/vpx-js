...
const isDmdName = n => n.includes('dmd')
const isCabVrName = n =>
	RE_VR.test(n) ||
	RE_CAB.test(n) ||
	n.includes('pincab') ||
...
export function postProcessScene(node, { viewerMode = 'viewer', harnessLog } = {}) {
	const stats = {
		vrKept: 0, cabForced: 0, lightmaps: 0, bumperLM: 0, vlmFixed: 0, metalFixed: 0,
		glass: 0, nanFixed: 0, nanHidden: 0, cabHidden: 0, cabFlipperHidden: 0, greenHidden: 0, playfieldHidden: 0,
	}
	...
	node.traverse(o => {
		...
		if (!o.isMesh) {
			...
			if (n && RE_CAB.test(n) && o.visible === false) { o.visible = true; stats.cabForced++ }
			if (n && RE_VR.test(n) && o.visible === false) { o.visible = true; stats.vrKept++ }
			return
		}
		const n = (o.name || '').toLowerCase()
		...
		if (viewerMode === 'play' && c.isVr && !c.isCab) {
			if (o.visible) { o.visible = false; stats.cabHidden++; try { o.geometry?.dispose?.() } catch {} }
			return
		}
		if (c.isVr) {
			if (o.visible === false) o.visible = true
			if (o.visible) { makeParentsVisible(o, node, stats); stats.vrKept++ }
		}
		if (c.isCab && !o.visible) { o.visible = true; stats.cabForced++; makeParentsVisible(o, node, stats) }
		if (!o.visible) return
		const mats = o.material ? (Array.isArray(o.material) ? o.material : [o.material]) : []
		if (mats.some(mm => { const mn = (mm?.name||'').toLowerCase(); const col = mm?.color?.getHexString?.()?.toLowerCase(); return mn.includes('green') || col === '00ff00' })) {
			hideMesh(o, 'greenHidden', stats); return
		}
		for (let i = 0; i < mats.length; i++) {
			...
			if (n.includes('flipper')) {
				...
				if (m.map && m.map.colorSpace !== THREE.SRGBColorSpace) try { m.map.colorSpace = THREE.SRGBColorSpace; m.map.needsUpdate = true } catch {}
				o.castShadow = false; o.receiveShadow = false
				continue
			}
			if (c.isVr || c.isCab) {
				const v = getVr(m)
				if (v !== m) { if (Array.isArray(o.material)) o.material[i] = v; else o.material = v; m = v }
				o.castShadow = false; o.receiveShadow = false; o.frustumCulled = true
				try { o.geometry?.computeBoundingSphere?.(); o.geometry?.computeBoundingBox?.() } catch {}
				stats.vlmFixed++
				continue
			}
			...
			if ((m.metalness > 0.3 || m.roughness < 0.4) && !m.name.toLowerCase().includes('ball')) {
				m.roughness = Math.max(m.roughness, 0.75); m.metalness = Math.min(m.metalness, 0.1); m.needsUpdate = true; stats.metalFixed++
			}
			if (n.includes('plastic') && !m.map && !pendingOf(m)) {
				m.side = THREE.DoubleSide; m.transparent = false; m.depthWrite = true; m.alphaTest = 0; m.opacity = 1
				m.roughness = Math.max(m.roughness ?? 0.6, 0.6); m.metalness = Math.min(m.metalness ?? 0, 0.05); m.needsUpdate = true
			}
			if (m.map && m.map.colorSpace !== THREE.SRGBColorSpace) try { m.map.colorSpace = THREE.SRGBColorSpace; m.map.needsUpdate = true } catch {}
		}
		o.receiveShadow = false
		if (c.isVr || c.isCab) { o.frustumCulled = true; try { o.geometry?.computeBoundingSphere?.(); o.geometry?.computeBoundingBox?.() } catch {} }
		if (n.includes('plastic') || n.includes('ramp')) for (const mat of (Array.isArray(o.material) ? o.material : [o.material])) mat.side = THREE.DoubleSide
	})

	let hasReadyBake = false, hasPendingBake = false
	node.traverse(o => {
		if (!o.isMesh) return
		const n = (o.name || '').toLowerCase()
	...
	node.traverse(o => { if (o.isMesh && o.visible && (o.name||'').toLowerCase().includes('bm_')) hasBakedDuplicate = true })
	...
	node.traverse(o => {
		...
		const m = Array.isArray(o.material) ? o.material[0] : o.material
		const c = m ? classify(n, (m.name||'').toLowerCase(), (m.map?.name||'').toLowerCase(), !!m.userData?.__isBaked) : { isGlass:false, isLm:false, isVlmBake:false, isMainBake:false }
		if (c.isGlass || c.isLm || (c.isVlmBake && !c.isMainBake)) return
		if (m && (m.name||'').toLowerCase().includes('green')) return
		o.visible = true; makeParentsVisible(o, node, stats)
	})
	node.traverse(o => {
		...
		o.visible = true; makeParentsVisible(o, node, stats); stats.cabForced++
	})
	...
	if (stats.playfieldHidden) harnessLog?.(`[playfield] Hid ${stats.playfieldHidden} base playfield (baked replaced)`, 'info')
	...
	const keepWhite = name => {
		...
		return n.includes('playfield') || n.includes('ball') || n.includes('flipper') || n.includes('bumper') || n.includes('light') || n.includes('dmd') || n.includes('vr_') || n.includes('vrcab') || n.includes('cabinet') || n.includes('lockbar') || n.includes('pincab') || n.includes('ramp') || n.includes('plastic') || n.includes('gate') || n.includes('kicker') || n.includes('target') || n.includes('spinner') || n.includes('primitive-bm') || n.includes('bm_')
	}
	node.traverse(o => {
		if (!o.isMesh || !o.visible) return
		const n = (o.name || '').toLowerCase()
		...
		hideMesh(o, null, stats); whiteWallsHidden++
	})
	if (whiteWallsHidden) harnessLog?.(`[wall] Hid ${whiteWallsHidden} white untextured stray meshes`, 'info')
	if (stats.nanFixed || stats.nanHidden) harnessLog?.(`[sanitize] ${stats.nanFixed} fixed ${stats.nanHidden} hidden`, 'info')
	return stats
}

export function ensureProceduralRoom(scene, center, size, opts = {}) {
	try { const e = scene.getObjectByName('vr_procedural_room'); if (e) scene.remove(e) } catch {}
	if (opts.hasVr) return null
	const maxDim = Math.max(size.x, size.y, size.z)
	const W = Math.max(1600, maxDim * 8), D = Math.max(1600, maxDim * 8), H = Math.max(900, maxDim * 6)
	const g = new THREE.Group()
	g.name = 'vr_procedural_room'
	const floorY = center.z - size.z * 0.6 - 50
	const box = new THREE.Mesh(new THREE.BoxGeometry(W, D, H), new THREE.MeshStandardMaterial({ color: 0x1c222e, roughness: 0.95, side: THREE.BackSide }))
	box.position.set(center.x, center.y, floorY + H * 0.5)
	g.add(box)
	const floor = new THREE.Mesh(new THREE.PlaneGeometry(W * 0.98, D * 0.98), new THREE.MeshStandardMaterial({ color: 0x232a3a, roughness: 0.92, side: THREE.DoubleSide }))
	...
	g.add(grid)
	try { const h = new THREE.HemisphereLight(0xffffff, 0x1a1f2e, 0.45); h.name = 'proc_hemi'; g.add(h) } catch {}
	scene.add(g)
	return g
}
...
function framingBox(node, exclude) {
	...
	node.traverse(o => {
		...
		try { box.expandByObject(o) } catch (e) { console.warn('[Box3]', o.name, e.message) }
	})
...
function framingState(node, targetExclude, sizeExclude, cfg) {
	...
	return {
		center: target.clone(), size, maxDim, target: target.clone(),
		position: new THREE.Vector3(target.x, target.y + dist * cfg.elev, target.z + dist * cfg.azim),
		near: Math.max(1, maxDim * cfg.near), far: Math.max(cfg.farMin, maxDim * cfg.farScale),
	}
}
...
export function applyCameraState(camera, controls, state) {
	...
	camera.near = state.near; camera.far = state.far
	camera.updateProjectionMatrix(); camera.lookAt(state.target); controls.update()
}
