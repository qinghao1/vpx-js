...
for (let i = 0; i < 90; i++) {
	...
	ready = await page
		.evaluate(() => document.getElementById('log')?.innerText.includes('Ready') ?? false)
		.catch(() => false)
	if (ready) break
}
if (!ready) {
...
const m = await page.evaluate(() => {
	...
	v.tableGroup.traverse(o => {
		if (o.isMesh && o.material)
			for (const mm of Array.isArray(o.material) ? o.material : [o.material]) mats.add(mm.uuid)
	})
	return {
...
for (const l of logs) {
	...
	if (a) parsed.parse = Number(a[1])
	const c = l.match(/Scene generated in (\d+)ms/)
	if (c) parsed.sceneGen = Number(c[1])
	const e = l.match(/Textures in scene: (\d+) ~([\d.]+) MB/)
	if (e) parsed.texScene = { c: Number(e[1]), mb: Number(e[2]) }
}
