...
export class BiffParser {
	...
	/** Reads int32 at 0. */
	protected getInt(buf: Uint8Array): number { return getDataView(buf).getInt32(0, true); }

	/** Reads float32 at 0 (single precision). */
	protected getFloat(buf: Uint8Array): number { return f4(getDataView(buf).getFloat32(0, true)); }

	/** Reads bool (int32 > 0). */
	protected getBool(buf: Uint8Array): boolean { return getDataView(buf).getInt32(0, true) > 0; }
	...
	private isAscii(s: string): boolean { return /^[\x00-\x7F]*$/.test(s); }
}
