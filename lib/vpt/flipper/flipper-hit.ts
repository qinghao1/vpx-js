...
export class FlipperHit extends HitObject {
	...
	public static getInstance(
		...
		if (data.flipperRadiusMin > 0 && data.flipperRadiusMax > data.flipperRadiusMin) {
			data.flipperRadius =
				data.flipperRadiusMax - (data.flipperRadiusMax - data.flipperRadiusMin) /* m_ptable->m_globalDifficulty*/
	...
	public contact(coll: CollisionEvent, dTime: number, physics: PlayerPhysics): void {
		...
		if (normVel <= C_CONTACTVEL) {
			...
			ball.hit.vel.addAndRelease(
				normal.clone(true).multiplyScalar(j * dTime * ball.hit.invMass - coll.hitOrgNormalVelocity),
			)
	...
	public collide(coll: CollisionEvent, physics: PlayerPhysics): void {
		...
		let impulse =
			(-(1.0 + epsilon) * bnv) / (ball.hit.invMass + normal.dotAndRelease(Vertex3D.crossProduct(pv1, rF, true)))
		...
		if (this.mover.isInContact) {
			if (rotI.z * this.mover.contactTorque < 0) {
	...
	public hitTestFlipperFace(ball: Ball, dTime: number, coll: CollisionEvent, face1: boolean): number {
		...
		if (
			...
			(k > C_INTERATIONS && Math.abs(bffnd) > ball.data.radius * 0.25)
		) {
		...
		const dist = Vertex2D.claim(
			// calculate moment from flipper base center
	...
	private hitTestFlipperEnd(ball: Ball, dTime: number, coll: CollisionEvent): number {
		...
		if (!isFinite(t) || t < 0 || t > dTime || (k > C_INTERATIONS && Math.abs(bFend) > ball.data.radius * 0.25)) {
