...
describe('The VPinball ball API', () => {
	...
	it('should correctly read and write the properties', async () => {
		...
		api.DecalMode = true
		expect(api.DecalMode).to.equal(true)
		api.DecalMode = false
		api.Mass = 1.6
		api.ID = 4
		api.Radius = 12
		api.BulbIntensityScale = 2.5
		api.ReflectionEnabled = true
		expect(api.ReflectionEnabled).to.equal(true)
		api.ReflectionEnabled = false
		api.PlayfieldReflectionScale = 3.5
		api.ForceReflection = true
		expect(api.ForceReflection).to.equal(true)
		api.ForceReflection = false
		api.Visible = true
		expect(api.Visible).to.equal(true)
		api.Visible = false
