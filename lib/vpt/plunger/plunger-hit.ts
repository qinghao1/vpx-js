...
import { Plunger, type PlungerConfig } from './plunger'
...
export class PlungerHit extends HitObject {
	...
	public hitTest(ball: Ball, dTime: number, coll: CollisionEvent, physics: PlayerPhysics): number {
		...
		if (isHit) {
			...
			if (coll.hitDistance <= 0.0 && coll.hitVel!.y === deltaY && Math.abs(deltaY) < Math.abs(coll.hitDistance)) {
	...
	public collide(coll: CollisionEvent, physics: PlayerPhysics): void {
		...
		if (coll.hitVel!.y !== 0.0) {
			...
			const reverseImpulseFudgeFactor = 0.22
			this.mover.reverseImpulse =
				ball.hit.vel.y * impulse * (ball.data.mass / this.mover.mass) * reverseImpulseFudgeFactor
		}
		...
		if (scatterVel > 0 && Math.abs(ball.hit.vel.y) > scatterVel) {
