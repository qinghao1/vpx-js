...
describe('The VPinball trigger API', () => {
	...
	it('should correctly read and write the properties', async () => {
		...
		trigger.X = 13735
		expect(trigger.X).to.equal(13735)
		trigger.Y = 53625
		expect(trigger.Y).to.equal(53625)
		trigger.Radius = 243.34
		expect(trigger.Radius).to.equal(243.34)
		trigger.Surface = 'sir face'
		expect(trigger.Surface).to.equal('sir face')
		trigger.Enabled = false
		expect(trigger.Enabled).to.equal(false)
		trigger.Enabled = true
		expect(trigger.Enabled).to.equal(true)
		trigger.Visible = false
		expect(trigger.Visible).to.equal(false)
		...
		expect(trigger.AnimSpeed).to.equal(3.5)
		trigger.Material = 'material world'
		expect(trigger.Material).to.equal('material world')
		trigger.TriggerShape = 3
		expect(trigger.TriggerShape).to.equal(3)
		trigger.ReflectionEnabled = false
		expect(trigger.ReflectionEnabled).to.equal(false)
		trigger.ReflectionEnabled = true
		expect(trigger.ReflectionEnabled).to.equal(true)

		trigger.Name = 'nahme me'
		expect(trigger.Name).to.equal('nahme me')
		trigger.TimerInterval = 231
		expect(trigger.TimerInterval).to.equal(231)
		trigger.TimerEnabled = false
		expect(trigger.TimerEnabled).to.equal(false)
		trigger.TimerEnabled = true
		expect(trigger.TimerEnabled).to.equal(true)
	})
