	...
	private async fromTag(buffer: Buffer, tag: string, offset: number, len: number): Promise<number> {
		switch (tag) {
			...
			case 'RADI':
				...
				break
			case 'MATR':
				this.szCapMaterial = this.getString(buffer, len)
				break
			case 'RIMA':
				this.szRingMaterial = this.getString(buffer, len)
				break
			case 'BAMA':
				this.szBaseMaterial = this.getString(buffer, len)
				break
			case 'SKMA':
				this.szSkirtMaterial = this.getString(buffer, len)
				break
			case 'THRS':
				this.threshold = this.getFloat(buffer)
				break
			case 'FORC':
				this.force = this.getFloat(buffer)
				break
			case 'BSCT':
				this.scatter = this.getFloat(buffer)
				break
			case 'HISC':
				this.heightScale = this.getFloat(buffer)
				break
			case 'RISP':
				this.ringSpeed = this.getFloat(buffer)
				break
			case 'ORIN':
				this.orientation = this.getFloat(buffer)
				break
			case 'RDLI':
				this.ringDropOffset = this.getFloat(buffer)
				break
			case 'SURF':
			...
			case 'CAVI':
				this.isCapVisible = this.getBool(buffer)
				break
			case 'HAHE':
			...
			case 'BSVS':
				...
				break
			case 'RIVS':
				this.isRingVisible = this.getBool(buffer)
				break
			case 'SKVS':
				this.isSkirtVisible = this.getBool(buffer)
				break
			case 'REEN':
