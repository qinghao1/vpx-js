...
export class Mesh {
	...
	private _writeVertexInfo(objFile: string[]): void {
		for (const vert of this.vertices) {
			objFile.push(
				`v ${fr(vert.x).toFixed(Mesh.exportPrecision)} ${fr(vert.y).toFixed(Mesh.exportPrecision)} ${fr(vert.z).toFixed(Mesh.exportPrecision)}`,
			)
		}
		for (const vert of this.vertices) {
		...
		for (const vert of this.vertices) {
			...
			const nz = vert.nz
			objFile.push(
				`vn ${fr(nx).toFixed(Mesh.exportPrecision)} ${fr(ny).toFixed(Mesh.exportPrecision)} ${fr(nz).toFixed(Mesh.exportPrecision)}`,
			)
		}
	}
