...
describe('regression: playfield lights via szLightmap', () => {
	it('primitive with szLightmap must react to light intensity even with empty state', async () => {
		...
		let node: any = null
		scene.traverse((o: any) => { if (o.name === 'TestPrim') node = o })
		...
		table.lights = {
			L1: {
				...
				state: { get intensity() { return curIntensity }, set intensity(v) { curIntensity = v } },
			},
		}
		...
		expect(mat.emissiveIntensity, 'emissive should be 1 when light on (alpha 100 * lightFactor 1 * 0.01)').toBeCloseTo(1, 0.01)
		// light OFF => effectiveIntensity 0
		curIntensity = 0
		prim.getUpdater().applyState(node, {}, api, table)
		mat = (node.children[0]?.material ?? node.material) as any
		expect(mat.emissiveIntensity, 'emissive must go to 0 when light off even with empty state (regression: Viewer must re-apply)').toEqual(0)
		// light half
		curIntensity = 5
		prim.getUpdater().applyState(node, {}, api, table)
		mat = (node.children[0]?.material ?? node.material) as any
	...
	it('Viewer.applyChangedStates must propagate to szLightmap primitives (file guard)', async () => {
		...
		expect(viewerCore, 'must call primitive applyState for lightmaps').toMatch(/entry\.item\.getUpdater\(\)\.applyState\(entry\.node,\s*\{\}/)
	})
	...
	it('walking_dead lightmap primitives should exist and be addBlend', async () => {
		...
		const vpxCandidates = [
			'walking_dead.vpx',
			'/home/qinghao1/Downloads/walking_dead.vpx',
		]
		const vpx = vpxCandidates.find(p => { try { return fs.existsSync(p) && fs.statSync(p).size > 1_000_000 } catch { return false } })
		if (!vpx) return
		...
		const lightmapped = Object.values(table.primitives as Record<string, any>).filter((p: any) => !!p.data.szLightmap)
		...
		const playfieldLM = lightmapped.filter((p: any) => (p.data.name ?? '').toLowerCase().includes('playfield') || (p.data.szImage ?? '').toLowerCase().includes('playfield'))
