...
describe('regression: texture stripe artifacts', () => {
	...
	it('playfield downscale must use box-average not point sample', () => {
		...
		for (let y=0;y<h;y++) for(let x=0;x<w;x++) for(let c=0;c<comps;c++) {
			const qx = Math.floor(x/2), qy=Math.floor(y/2)
		...
		for (let y=0;y<2;y++) for(let x=0;x<2;x++) for(let c=0;c<comps;c++) {
