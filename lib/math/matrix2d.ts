...
export class Matrix2D {
	...
	public multiplyMatrix(m1: Matrix2D, m2: Matrix2D) {
		for (let i = 0; i < 3; ++i) {
			for (let l = 0; l < 3; ++l) {
				this.matrix[i][l] =
					m1.matrix[i][0] * m2.matrix[0][l] + m1.matrix[i][1] * m2.matrix[1][l] + m1.matrix[i][2] * m2.matrix[2][l]
			}
		}
	}
	...
	public toString() {
		return (
			`[${Math.round(this.matrix[0][0] * 1000) / 1000}, ${Math.round(this.matrix[0][1] * 1000) / 1000}, ${Math.round(this.matrix[0][2] * 1000) / 1000}]\n` +
