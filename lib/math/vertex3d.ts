...
export class Vertex3D implements Vertex {
	...
	set z(v: number) { this._z = f4(v); }

	constructor(x?: number, y?: number, z?: number) {
		this._x = f4(x ?? 0); this._y = f4(y ?? 0); this._z = f4(z ?? 0);
	}
	...
	static from(data: any): Vertex3D { return Object.assign(new Vertex3D(), data); }
	...
	static reset(v: Vertex3D): void { v.set(0, 0, 0); }

	set(v: Vertex3D): this;
	set(x: number, y: number, z?: number): this;
	set(xOrV: number | Vertex3D, y?: number, z?: number): this {
		if (typeof xOrV === 'number') {
			this._x = f4(xOrV); this._y = f4(y!); this._z = f4(z ?? 0);
		} else {
			this._x = f4(xOrV.x); this._y = f4(xOrV.y); this._z = f4(xOrV.z);
		}
		return this;
	}

	/** Copies and releases source. */
	setAndRelease(v: Vertex3D): this { this.set(v); Vertex3D.release(v); return this; }
	...
	normalizeSafe(): void { if (!this.isZero()) this.normalize(); }

	length(): number { return f4(new Vector3(this._x, this._y, this._z).length()); }

	lengthSq(): number { return f4(new Vector3(this._x, this._y, this._z).lengthSq()); }

	divideScalar(s: number): this { return this.multiplyScalar(f4(1 / s)); }

	multiplyScalar(s: number): this {
		const f = f4(s); this._x = f4(this._x * f); this._y = f4(this._y * f); this._z = f4(this._z * f); return this;
	}
	...
	dot(v: Vertex3D): number { return f4(new Vector3(this._x, this._y, this._z).dot(new Vector3(v._x, v._y, v._z))); }

	/** Dot and releases source. */
	dotAndRelease(v: Vertex3D): number { const d = this.dot(v); Vertex3D.release(v); return d; }

	sub(v: Vertex3D): this {
		this._x = f4(this._x - v._x); this._y = f4(this._y - v._y); this._z = f4(this._z - v._z); return this;
	}

	/** Subtracts and releases source. */
	subAndRelease(v: Vertex3D): this { this.sub(v); Vertex3D.release(v); return this; }
	...
	addAndRelease(v: Vertex3D): this { this.add(v); Vertex3D.release(v); return this; }

	cross(v: Vertex3D): this { return this.crossVectors(this, v); }
	...
	crossVectors(a: Vertex3D, b: Vertex3D): this {
		...
		this._x = f4(out.x); this._y = f4(out.y); this._z = f4(out.z); return this;
	}

	/** Returns XY as Vertex2D. */
	xy(): Vertex2D { return new Vertex2D(this._x, this._y); }

	/** Sets to zero. */
	setZero(): this { return this.set(0, 0, 0); }

	/** Checks near-zero. */
	isZero(): boolean { return Math.abs(this._x) < FLT_MIN && Math.abs(this._y) < FLT_MIN && Math.abs(this._z) < FLT_MIN; }

	/** Exact equality. */
	equals(v: Vertex3D): boolean { return v._x === this._x && v._y === this._y && v._z === this._z; }
	...
	static getRotatedAxis(angle: number, axis: Vertex3D, temp: Vertex3D): Vertex3D {
		...
		const s = f4(Math.sin(rad)), c = f4(Math.cos(rad)), omc = f4(1 - c);
		const r0 = new Vertex3D(f4(u._x * u._x + c * (1 - u._x * u._x)), f4(u._x * u._y * omc - s * u._z), f4(u._x * u._z * omc + s * u._y));
		const r1 = new Vertex3D(f4(u._x * u._y * omc + s * u._z), f4(u._y * u._y + c * (1 - u._y * u._y)), f4(u._y * u._z * omc - s * u._x));
		const r2 = new Vertex3D(f4(u._x * u._z * omc - s * u._y), f4(u._y * u._z * omc + s * u._x), f4(u._z * u._z + c * (1 - u._z * u._z)));
		Vertex3D.release(u);
	...
	toThree(): Vector3 { return new Vector3(this._x, this._y, this._z); }

	/** Creates from THREE.Vector3. */
	static fromThree(v: Vector3): Vertex3D { return new Vertex3D(v.x, v.y, v.z); }
}
