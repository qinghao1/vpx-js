...
export class HitQuadtree {
	...
	public addElement(o: HitObject): void {
		this.vho.push(o)
	}
	...
	public initialize(b?: FRect3D): void {
		if (!b) {
			b = new FRect3D()
			for (const h of this.vho) b.extend(h.hitBBox)
		}
		const warm = () => {
			let circleCount = 0,
				planeCount = 0,
	...
	private hitTestBallScalar(ball: Ball, coll: CollisionEvent, physics: PlayerPhysics): void {
		...
		for (let i = 0; i < this.vho.length; i++) {
			const h = this.vho[i]!
			if (h !== ball.hit && h.hitBBox.intersectRect(b) && h.hitBBox.intersectSphere(pos, rs))
				h.doHitTest(ball, coll, physics)
		}
		if (this.isLeaf) return
		const { x, y } = this.vCenter,
			left = b.left <= x,
			right = b.right >= x
		if (b.top <= y) {
			if (left) this.children[0]?.hitTestBall(ball, coll, physics)
			if (right) this.children[1]?.hitTestBall(ball, coll, physics)
		}
		if (b.bottom >= y) {
			if (left) this.children[2]?.hitTestBall(ball, coll, physics)
			if (right) this.children[3]?.hitTestBall(ball, coll, physics)
		}
	}
	...
	private traverse(node: HitQuadtree, ball: Ball): void {
		...
		for (let i = 0; i < node.vho.length; i++) {
			...
			if (
				...
				continue
			if (isBatchCircle(h)) {
				this.pushOrder(h, HitKind.Circle, circles.length)
				circles.push(h)
			} else if (h.hitKind === HitKind.Plane) {
				this.pushOrder(h, HitKind.Plane, planes.length)
				planes.push(h as HitPlane)
			} else if (h.hitKind === HitKind.LineZ) {
				this.pushOrder(h, HitKind.LineZ, lineZs.length)
				lineZs.push(h as HitLineZ)
			} else if (isBatchPoint(h)) {
				this.pushOrder(h, HitKind.Point, points.length)
				points.push(h)
			} else if (isBatchTriangle(h)) {
				this.pushOrder(h, HitKind.Triangle, triangles.length)
				triangles.push(h)
			} else if (isBatchLine3D(h)) {
				this.pushOrder(h, HitKind.Line3D, line3Ds.length)
				line3Ds.push(h)
			} else if (isBatchLineSeg(h)) {
				this.pushOrder(h, HitKind.LineSeg, lineSegs.length)
				lineSegs.push(h)
			} else if (isBatchPoly(h)) {
				this.pushOrder(h, HitKind.Poly, polys.length)
				polys.push(h)
		...
		const b = ball.hit.hitBBox,
			...
			right = b.right >= x
		if (b.top <= y) {
			if (left && node.children[0]) this.traverse(node.children[0], ball)
			if (right && node.children[1]) this.traverse(node.children[1], ball)
		}
		if (b.bottom >= y) {
			if (left && node.children[2]) this.traverse(node.children[2], ball)
			if (right && node.children[3]) this.traverse(node.children[3], ball)
		}
	}
