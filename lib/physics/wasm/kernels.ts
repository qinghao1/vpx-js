type Mod = {
	...
	_batchHitTestPoint: (n: number, bx: number, by: number, bz: number, vx: number, vy: number, vz: number, r: number, px: number, py: number, pz: number, dt: number, oT: number, oContact: number, oNx: number, oNy: number, oNz: number, oDist: number, oBnv: number) => void
...
export async function getWasmKernels(): Promise<Mod> {
	...
	loading = (async () => {
		...
		for (const rel of ['../../../wasm/kernels/dist/kernels.js', '../../../../wasm/kernels/dist/kernels.js']) {
			try {
...
export type PointViews = { px: Float32Array; py: Float32Array; pz: Float32Array } & HitOut & { run: (bx: number, by: number, bz: number, vx: number, vy: number, vz: number, r: number, dt: number) => void }
...
const pointSoa: Soa<PointViews> = { in: { cap: 0, ptrs: null }, out: { cap: 0, ptrs: null }, views: null, buf: null, viewCap: 0, n: 0 }
...
export function tryGetWasmBatchHitViewsOutCircle(count: number): CircleViews | null { return tryGet(circleSoa, count) }
export function tryGetWasmBatchHitViewsOutPlane(count: number): PlaneViews | null { return tryGet(planeSoa, count) }
export function tryGetWasmBatchHitViewsOutLineZ(count: number): LineViews | null { return tryGet(lineSoa, count) }
export function tryGetWasmBatchHitViewsOutPoint(count: number): PointViews | null { return tryGet(pointSoa, count) }
...
export function getWasmBatchHitViewsOutPoint(count: number): PointViews {
	...
	if (!pointSoa.views || pointSoa.buf !== m.HEAPF32.buffer || pointSoa.viewCap !== pointSoa.in.cap) {
		...
		pointSoa.views = {
			...
			...out,
			run: (bx, by, bz, vx, vy, vz, r, dt) => getWasmModSync()!._batchHitTestPoint(pointSoa.n, bx, by, bz, vx, vy, vz, r, ip.px!, ip.py!, ip.pz!, dt, op.oT!, op.oContact!, op.oNx!, op.oNy!, op.oNz!, op.oDist!, op.oBnv!),
		}
...
export function getWasmBatchHitViewsOutTriangle(count: number): TriangleViews {
	...
	if (!triangleSoa.views || triangleSoa.buf !== m.HEAPF32.buffer || triangleSoa.viewCap !== triangleSoa.in.cap) {
		...
		triangleSoa.views = {
			...
			...out,
			run: (bx, by, bz, vx, vy, vz, r, dt) => getWasmModSync()!._batchHitTestTriangle(triangleSoa.n, bx, by, bz, vx, vy, vz, r, ip.r0x!, ip.r0y!, ip.r0z!, ip.r1x!, ip.r1y!, ip.r1z!, ip.r2x!, ip.r2y!, ip.r2z!, ip.nx!, ip.ny!, ip.nz!, dt, op.oT!, op.oContact!, op.oNx!, op.oNy!, op.oNz!, op.oDist!, op.oBnv!),
		}
...
export function getWasmBatchHitViewsOutLineSeg(count: number): LineSegViews {
	...
	if (!lineSegSoa.views || lineSegSoa.buf !== m.HEAPF32.buffer || lineSegSoa.viewCap !== lineSegSoa.in.cap) {
		...
		lineSegSoa.views = {
			...
			...out,
			run: (bx, by, bz, vx, vy, vz, r, dt) => getWasmModSync()!._batchHitTestLineSeg(lineSegSoa.n, bx, by, bz, vx, vy, vz, r, ip.v1x!, ip.v1y!, ip.v2x!, ip.v2y!, ip.nx!, ip.ny!, ip.len!, ip.zl!, ip.zh!, dt, op.oT!, op.oContact!, op.oNx!, op.oNy!, op.oNz!, op.oDist!, op.oBnv!),
		}
...
export function getWasmBatchHitViewsOutLine3D(count: number): Line3DViews {
	...
	if (!line3DSoa.views || line3DSoa.buf !== m.HEAPF32.buffer || line3DSoa.viewCap !== line3DSoa.in.cap) {
		...
		line3DSoa.views = {
			...
			...out,
			run: (bx, by, bz, vx, vy, vz, r, dt) => getWasmModSync()!._batchHitTestLine3D(line3DSoa.n, bx, by, bz, vx, vy, vz, r, ip.lx!, ip.ly!, ip.zl!, ip.zh!, ip.m00!, ip.m01!, ip.m02!, ip.m10!, ip.m11!, ip.m12!, ip.m20!, ip.m21!, ip.m22!, dt, op.oT!, op.oContact!, op.oNx!, op.oNy!, op.oNz!, op.oDist!, op.oBnv!),
		}
