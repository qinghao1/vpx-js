...
import type { HitLine3D } from './hit-line-3d.js'
import type { HitLineZ } from './hit-line-z.js'
import { HitKind, type HitObject } from './hit-object.js'
import type { HitPlane } from './hit-plane.js'
import type { HitPoint } from './hit-point.js'
import type { HitTriangle } from './hit-triangle.js'
...
import type {
	...
	PolyViews,
	TriangleViews,
} from './wasm/kernels.js'
import {
	isWasmReady,
	POLY_MAX_VERTS,
	tryGetWasmBatchHitViewsOutCircle,
	tryGetWasmBatchHitViewsOutLine3D,
	tryGetWasmBatchHitViewsOutLineSeg,
	tryGetWasmBatchHitViewsOutLineZ,
	tryGetWasmBatchHitViewsOutPlane,
	tryGetWasmBatchHitViewsOutPoint,
	tryGetWasmBatchHitViewsOutPoly,
	tryGetWasmBatchHitViewsOutTriangle,
	warmWasmPools,
} from './wasm/kernels.js'
...
const isBatchCircle = (h: HitObject): h is HitCircle =>
...
const isBatchLineSeg = (h: HitObject): h is LineSeg =>
...
const isBatchPoly = (h: HitObject): h is Hit3DPoly =>
...
export class HitKDNode {
	...
	public hitTestBall(ball: Ball, coll: CollisionEvent, physics: PlayerPhysics): void {
		...
		const circleViews = circles.length ? tryGetWasmBatchHitViewsOutCircle(circles.length) : null
		const planeViews = planes.length ? tryGetWasmBatchHitViewsOutPlane(planes.length) : null
		const lineViews = lineZs.length ? tryGetWasmBatchHitViewsOutLineZ(lineZs.length) : null
		const pointViews = points.length ? tryGetWasmBatchHitViewsOutPoint(points.length) : null
		const triangleViews = triangles.length ? tryGetWasmBatchHitViewsOutTriangle(triangles.length) : null
		const lineSegViews = lineSegs.length ? tryGetWasmBatchHitViewsOutLineSeg(lineSegs.length) : null
		const line3DViews = line3Ds.length ? tryGetWasmBatchHitViewsOutLine3D(line3Ds.length) : null
		const polyViews = polys.length ? tryGetWasmBatchHitViewsOutPoly(polys.length) : null
		if (
			(circles.length && !circleViews) ||
			(planes.length && !planeViews) ||
			(lineZs.length && !lineViews) ||
			(points.length && !pointViews) ||
			(triangles.length && !triangleViews) ||
			(lineSegs.length && !lineSegViews) ||
	...
	private fillCircles(circleViews: CircleViews, circles: HitCircle[]): void {
		for (let i = 0; i < circles.length; i++) {
			const h = circles[i]!
			circleViews.cx[i] = h.center.x
			circleViews.cy[i] = h.center.y
			circleViews.cr[i] = h.radius
			circleViews.zl[i] = h.hitBBox.zlow
			circleViews.zh[i] = h.hitBBox.zhigh
		}
	}
	private fillPlanes(planeViews: PlaneViews, planes: HitPlane[]): void {
		for (let i = 0; i < planes.length; i++) {
			const h = planes[i]!
			planeViews.nx[i] = h.normal.x
			planeViews.ny[i] = h.normal.y
			planeViews.nz[i] = h.normal.z
			planeViews.d[i] = h.d
		}
	}
	private fillLineZs(lineViews: LineViews, lineZs: HitLineZ[]): void {
		for (let i = 0; i < lineZs.length; i++) {
			const h = lineZs[i]!
			lineViews.lx[i] = h.xy.x
			lineViews.ly[i] = h.xy.y
			lineViews.zl[i] = h.hitBBox.zlow
			lineViews.zh[i] = h.hitBBox.zhigh
		}
	}
	private fillPoints(pointViews: PointViews, points: HitPoint[]): void {
		for (let i = 0; i < points.length; i++) {
			const h = points[i]!
			pointViews.px[i] = h.p.x
			pointViews.py[i] = h.p.y
			pointViews.pz[i] = h.p.z
		}
	}
	private fillTriangles(triangleViews: TriangleViews, triangles: HitTriangle[]): void {
		for (let i = 0; i < triangles.length; i++) {
			...
			const r0 = h.rgv[0]!,
				r1 = h.rgv[1]!,
				r2 = h.rgv[2]!
			triangleViews.r0x[i] = r0.x
			triangleViews.r0y[i] = r0.y
			triangleViews.r0z[i] = r0.z
			triangleViews.r1x[i] = r1.x
			triangleViews.r1y[i] = r1.y
			triangleViews.r1z[i] = r1.z
			triangleViews.r2x[i] = r2.x
			triangleViews.r2y[i] = r2.y
			triangleViews.r2z[i] = r2.z
			triangleViews.nx[i] = h.normal.x
			triangleViews.ny[i] = h.normal.y
			triangleViews.nz[i] = h.normal.z
		}
	}
	private fillLineSegs(lineSegViews: LineSegViews, lineSegs: LineSeg[]): void {
		for (let i = 0; i < lineSegs.length; i++) {
			const h = lineSegs[i]!
			lineSegViews.v1x[i] = h.v1.x
			lineSegViews.v1y[i] = h.v1.y
			lineSegViews.v2x[i] = h.v2.x
			lineSegViews.v2y[i] = h.v2.y
			lineSegViews.nx[i] = h.normal.x
			lineSegViews.ny[i] = h.normal.y
			lineSegViews.len[i] = h.length
			lineSegViews.zl[i] = h.hitBBox.zlow
			lineSegViews.zh[i] = h.hitBBox.zhigh
		}
	}
	private fillLine3Ds(line3DViews: Line3DViews, line3Ds: HitLine3D[]): void {
		for (let i = 0; i < line3Ds.length; i++) {
			const h = line3Ds[i]!
			const m = h.matrix.elements
			line3DViews.lx[i] = h.xy.x
			line3DViews.ly[i] = h.xy.y
			line3DViews.zl[i] = h.zLow
			line3DViews.zh[i] = h.zHigh
			line3DViews.m00[i] = m[0]!
			line3DViews.m01[i] = m[3]!
			line3DViews.m02[i] = m[6]!
			line3DViews.m10[i] = m[1]!
			line3DViews.m11[i] = m[4]!
			line3DViews.m12[i] = m[7]!
			line3DViews.m20[i] = m[2]!
			line3DViews.m21[i] = m[5]!
			line3DViews.m22[i] = m[8]!
		}
	}
	private fillPolys(polyViews: PolyViews, polys: Hit3DPoly[]): void {
		for (let i = 0; i < polys.length; i++) {
			const h = polys[i]!
			polyViews.nx[i] = h.normal.x
			polyViews.ny[i] = h.normal.y
			polyViews.nz[i] = h.normal.z
			const r0 = h.rgv[0]!
			polyViews.r0x[i] = r0.x
			polyViews.r0y[i] = r0.y
			polyViews.r0z[i] = r0.z
			polyViews.numVerts[i] = h.rgv.length
			const base = i * POLY_MAX_VERTS
			for (let j = 0; j < h.rgv.length; j++) {
				polyViews.vertsX[base + j] = h.rgv[j]!.x
				polyViews.vertsY[base + j] = h.rgv[j]!.y
			}
		}
	}
	...
	private replay(
		...
		circleViews: CircleViews | null,
		planeViews: PlaneViews | null,
		lineViews: LineViews | null,
		pointViews: PointViews | null,
		triangleViews: TriangleViews | null,
		lineSegViews: LineSegViews | null,
		line3DViews: Line3DViews | null,
		polyViews: PolyViews | null,
	): void {
		for (let i = 0; i < this._orderLen; i++) {
			const e = this._order[i]!
			if (e.kind === HitKind.Other) {
				e.obj.doHitTest(ball, coll, physics)
				continue
			}
			const s =
				e.kind === HitKind.Circle
					? circleViews!
					: e.kind === HitKind.Plane
						? planeViews!
						: e.kind === HitKind.LineZ
							? lineViews!
							: e.kind === HitKind.Point
								? pointViews!
								: e.kind === HitKind.Triangle
									? triangleViews!
									: e.kind === HitKind.LineSeg
										? lineSegViews!
										: e.kind === HitKind.Line3D
											? line3DViews!
											: polyViews!
			const t = s.oT[e.idx]!,
				contact = s.oContact[e.idx]!,
				nx = s.oNx[e.idx]!,
				ny = s.oNy[e.idx]!,
				nz = s.oNz[e.idx]!,
				dist = s.oDist[e.idx]!,
			...
			if (!physics.recordContacts) {
				...
				nc.hitNormal.z = nz
				nc.hitDistance = dist
				nc.isContact = isContact
				nc.hitOrgNormalVelocity = bnv
				if (isContact || valid) {
					nc.ball = ball
					nc.obj = e.obj
	...
	private hitTestBallScalar(ball: Ball, coll: CollisionEvent, physics: PlayerPhysics): void {
		...
		for (let i = this.start; i < this.start + count; i++) {
			const h = this.hitOct.getItemAt(i)
			if (h !== ball.hit) h.doHitTest(ball, coll, physics)
		}
		if (this.children.length === 0) return
		const axis = this.items >> AXIS_SHIFT,
			bounds = this.rectBounds,
			box = ball.hit.hitBBox
		if (axis === 0) {
	...
	private collect(ball: Ball): number {
		...
		this._lineZs.length = 0
		this._points.length = 0
		this._triangles.length = 0
		this._lineSegs.length = 0
		this._line3Ds.length = 0
	...
	private traverse(node: HitKDNode, ball: Ball): void {
		const r = node.rectBounds,
			b = ball.hit.hitBBox,
			pos = ball.state.pos,
			rs = ball.hit.rcHitRadiusSqr
		...
		for (let i = node.start; i < node.start + count; i++) {
			...
			if (isBatchCircle(h)) {
				this.pushOrder(h, HitKind.Circle, this._circles.length)
				this._circles.push(h)
			} else if (h.hitKind === HitKind.Plane) {
				this.pushOrder(h, HitKind.Plane, this._planes.length)
				this._planes.push(h as HitPlane)
			} else if (h.hitKind === HitKind.LineZ) {
				this.pushOrder(h, HitKind.LineZ, this._lineZs.length)
				this._lineZs.push(h as HitLineZ)
			} else if (isBatchPoint(h)) {
				this.pushOrder(h, HitKind.Point, this._points.length)
				this._points.push(h)
			} else if (isBatchTriangle(h)) {
				this.pushOrder(h, HitKind.Triangle, this._triangles.length)
				this._triangles.push(h)
			} else if (isBatchLine3D(h)) {
				this.pushOrder(h, HitKind.Line3D, this._line3Ds.length)
				this._line3Ds.push(h)
			} else if (isBatchLineSeg(h)) {
				this.pushOrder(h, HitKind.LineSeg, this._lineSegs.length)
				this._lineSegs.push(h)
			} else if (isBatchPoly(h)) {
				this.pushOrder(h, HitKind.Poly, this._polys.length)
				this._polys.push(h)
		...
		if (axis === 0) {
			const cc = (r.left + r.right) * 0.5
			if (b.left <= cc) this.traverse(node.children[0]!, ball)
			if (b.right >= cc) this.traverse(node.children[1]!, ball)
		} else if (axis === 1) {
			const cc = (r.top + r.bottom) * 0.5
			if (b.top <= cc) this.traverse(node.children[0]!, ball)
			if (b.bottom >= cc) this.traverse(node.children[1]!, ball)
		} else {
			const cc = (r.zlow + r.zhigh) * 0.5
			if (b.zlow <= cc) this.traverse(node.children[0]!, ball)
			if (b.zhigh >= cc) this.traverse(node.children[1]!, ball)
		}
	}
	...
	public createNextLevel(level: number, levelEmpty: number): void {
		...
		if (axis === 0) {
			this.children[0].rectBounds.right = vcX
			this.children[1].rectBounds.left = vcX
		} else if (axis === 1) {
			this.children[0].rectBounds.bottom = vcY
			this.children[1].rectBounds.top = vcY
		} else {
			this.children[0].rectBounds.zhigh = vcZ
			this.children[1].rectBounds.zlow = vcZ
		}
		...
		if (axis === 0) {
			for (let i = this.start; i < this.start + org; i++) {
				...
				if (h.right < vcX) this.children[0].items++
				else if (h.left > vcX) this.children[1].items++
			}
		} else if (axis === 1) {
			for (let i = this.start; i < this.start + org; i++) {
				const h = this.hitOct.getItemAt(i).hitBBox
				if (h.bottom < vcY) this.children[0].items++
				else if (h.top > vcY) this.children[1].items++
			}
		} else {
			for (let i = this.start; i < this.start + org; i++) {
				const h = this.hitOct.getItemAt(i).hitBBox
				if (h.zhigh < vcZ) this.children[0].items++
				else if (h.zlow > vcZ) this.children[1].items++
			}
		}
		const leftCount = this.children[0].items,
			rightCount = this.children[1].items
		let levelEmptyLocal = levelEmpty,
			middleCount = org - leftCount - rightCount,
			countEmpty = 0
		...
		if (axis === 0) {
			for (let i = this.start; i < this.start + org; i++) {
				...
				const h = this.hitOct.getItemAt(i).hitBBox
				if (h.right < vcX) this.hitOct.tmp[this.children[0].start + this.children[0].items++] = idx
				else if (h.left > vcX) this.hitOct.tmp[this.children[1].start + this.children[1].items++] = idx
			...
			for (let i = this.start; i < this.start + org; i++) {
				const idx = this.hitOct.orgIdx[i]!
				const h = this.hitOct.getItemAt(i).hitBBox
				if (h.bottom < vcY) this.hitOct.tmp[this.children[0].start + this.children[0].items++] = idx
				else if (h.top > vcY) this.hitOct.tmp[this.children[1].start + this.children[1].items++] = idx
				else this.hitOct.orgIdx[this.start + middle++] = idx
			}
		} else {
			for (let i = this.start; i < this.start + org; i++) {
				const idx = this.hitOct.orgIdx[i]!
				const h = this.hitOct.getItemAt(i).hitBBox
				if (h.zhigh < vcZ) this.hitOct.tmp[this.children[0].start + this.children[0].items++] = idx
				else if (h.zlow > vcZ) this.hitOct.tmp[this.children[1].start + this.children[1].items++] = idx
				else this.hitOct.orgIdx[this.start + middle++] = idx
			}
		}
