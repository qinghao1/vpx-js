...
describe('The ROM Fetcher', () => {
	...
	it('should successful download ROM', async () => {
		const nockScopeA = nock('https://api.vpdb.io').get('/v1/games/mm/roms/').reply(200, VPDB_GAME_ENTRY_JSON)

		const nockScopeB = nock('https://storage.vpdb.io').get('/files/p2b9gpvd1.zip/mm_1_09c.bin').reply(200, MOCK_ROM)
